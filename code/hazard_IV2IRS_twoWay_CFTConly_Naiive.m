...
link_choice    = 'cloglog';  % Cloglog Link Function
...
F = readtable(fitfile);
needF = {'FundID','Year','Month','yhat_within'};
assert(all(ismember(needF, F.Properties.VariableNames)), ...
    'Fitted file must have: FundID, Year, Month, yhat_within');
...
% Volatility: 12m rolling std of Return, lagged
...
writetable(M, 'hazard_merged_CFTC_Naiive.csv');

% Drop missing key regressors
keyvars = {'Flow','ret_L1','vol_L1','lnAUM_L1','Event'};
ok = true(height(M),1);
...
H = M(ok,:);
writetable(H, 'hazard_panel_CFTC_Naiive.csv');
...
% Key regressors
Xcore = [ones(height(H),1), H.Flow,  H.ret_L1, H.vol_L1, H.lnAUM_L1];
core_names = {'Intercept','Flow','ret_L1','vol_L1','lnAUM_L1'}; % Without including Residual from 1st-stage regression
X = [sparse(Xcore), Dt];
y = H.Event;
...
writetable(Tcoef, 'hazard_coef_twoWay_CFTC_Naiive.csv');
...
writetable(Tame, 'hazard_ame_CFTC_Naiive.csv');

fid = fopen('hazard_summary_Naiive.txt','w');
