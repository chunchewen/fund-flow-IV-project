...
Flex = mean([0.40*Lc, 0.30*Nc, 0.15*Rc, 0.15*Sc], 2, 'omitnan');   % [0,1]
...
StyleBoost(ismember(styleNum, derivCodes)) = 0.20;

flagBlock = mean([0.30*Futures, 0.30*Derivs, 0.20*Margin, 0.20*FXCredit], 2, 'omitnan');
DerivativesIntensity = clamp01(flagBlock + 0.25*LevScaled + StyleBoost);

% (3) HNW SCORE FLAGS 
minInv    = col('MinimumInvestment');      
...
%  CFTC 4.13(a)(4) rescission -> compliance on from 2013-01
...
E_tax_C = clamp01(0.60*HNW_score + 0.20*OpenToPublic + 0.20*AccptsManagedAcc);
...
% Add all Z_* columns
addZ = struct( ...
 'Z_CFTC_q',Z_CFTC_q, 'Z_FATCA_q',Z_FATCA_q, ...
 'Z_TaxWin1997_A',Z_TaxWin1997_A,'Z_TaxWin2003_A',Z_TaxWin2003_A,'Z_TaxWin2013_A',Z_TaxWin2013_A, ...
