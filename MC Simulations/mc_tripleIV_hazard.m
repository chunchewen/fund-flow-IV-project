...
%% -------------------- Simulation settings --------------------
M      = 800;     % Monte Carlo replications
N      = 1200;    % funds
T      = 60;      % months
...
rho_U   =  0.70;   % unobserved factor loading in Flow (endogeneity)
...
for m = 1:M
    ...
    e    = sigma_e*randn(N,T);
    Flow = pi0 + piZ*Z + (piX(1)*ret + piX(2)*vol + piX(3)*lnAUM) + rho_U*U + e;

    % ---- Hazard DGP (comploglog) ----
    theta = 0.50;                     % U enters hazard too
...
% Delete (remove) values outside [p1, p99]
estA = estA(estA >= prctile(estA,1) & estA <= prctile(estA,99));
estB = estB(estB >= prctile(estB,1) & estB <= prctile(estB,99));
seA = seA(seA >= prctile(seA,1) & seA <= prctile(seA,99));
seB = seB(seB >= prctile(seB,1) & seB <= prctile(seB,99));

%% -------------------- Summaries --------------------
R_A = summarize('Naive comploglog (no IV)', estA, seA, b_flow, alpha);
R_B = summarize('Control-function comploglog (IV)', estB, seB, b_flow, alpha);

fprintf('\nFirst-stage strength (instrument Z | controls):\n');
fprintf('  Mean F-stat: %.2f   Median F-stat: %.2f\n', mean(F1), median(F1));

% Optional figure (safe in MATLAB; comment out if running headless)
figure('Color','w'); hold on;
histogram(estA,60,'FaceAlpha',0.35,'DisplayName','Naive (no IV)');
histogram(estB,60,'FaceAlpha',0.35,'DisplayName','CF comploglog (IV)');
xline(b_flow,'k--','LineWidth',1.5,'DisplayName','True b_{flow}');
xlabel('Estimated coefficient on Flow'); ylabel('Frequency');
title('Figure 1: Comparison of Flow effects on liquidation hazard');
legend('Location','best'); box on;

% Print concise summary
fprintf('\n=== Monte Carlo Summary (M=%d) ===\n', M);
disp(struct('FirstStageF_mean',mean(F1),'FirstStageF_median',median(F1),...
            'Naive',R_A,'IV_CF',R_B));
...
function R = summarize(name, est, se, trueVal, alpha)
% Summarize Monte Carlo distribution of the Flow coefficient
z = (est - trueVal)./se;
zcrit = norminv(1 - alpha/2);
cover = (est - zcrit.*se <= trueVal) & (trueVal <= est + zcrit.*se);
R = struct();
R.name    = name;
R.mean    = mean(est);
R.bias    = mean(est) - trueVal;
R.rmse    = sqrt(mean( (est-trueVal).^2 ));
R.se_mean = mean(se);
R.cover   = mean(cover);
fprintf('\n[%s]\n', name);
fprintf('  True b_flow = %.3f | Mean est = %.3f | Bias = %.3f | RMSE = %.3f | Mean SE = %.3f | 95%% Cover = %.3f\n',...
        trueVal, R.mean, R.bias, R.rmse, R.se_mean, R.cover);
end
