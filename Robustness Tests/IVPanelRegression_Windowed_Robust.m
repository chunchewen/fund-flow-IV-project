%% This code conduct the robust first-stage IV regression with equal weights for relevant IVs
clear; clc;
run_Flow_t   = true;    
lead_horizons = [6,12];

ym = @(Y,M) Y*12 + M;   % month-id
...
% -------- Window and Pre-period functions (apply on any table) --------
winfun = struct();
...
% Conduct Robust 1st-stage regressions
if run_Flow_t
    run_spec(T, 'Flow', ivars, controls, 'Flow_spec_Robust', lead_horizons, winfun, prefun, ym);
end
