% This code conduct Robustness test for the first-stage IV regression based on wider window %

clear; clc;
run_Flow_t   = true;    % Spec A: y=Flow_t with Z_t
lead_horizons = [6,12];

ym = @(Y,M) Y*12 + M;   % For month-id

T = readtable('C:\Users\ias05106\OneDrive - University of Strathclyde\Desktop\LipperTASS2018\FundFlowWork\Revision2025\RevisedIVdummy\RobustnessTests\performance_with_instruments.csv');
...
T(T.Year <= 2000 | T.Year >= 2017, :)=[];  % This is windowed estimation
...
% Conduct full 1st-stage specification
function run_spec(T, yvar, ivars, controls, tag, lead_horizons, winfun, prefun, ym)
    ...
    % -------- Windowed F-tests (mask computed on D) --------
    ...
    % -------- ALL_LEADS placebo (robust mapping) --------
    leadNames = {};
    for L = lead_horizons
...
% Conduct 1st-stage specifications
if run_Flow_t
    run_spec(T, 'Flow', ivars, controls, 'Flow_spec_v9b', lead_horizons, winfun, prefun, ym);
end
